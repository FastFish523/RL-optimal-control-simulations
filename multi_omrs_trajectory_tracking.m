...
for i = 1:num_weights
    w1_plot_handles(i) = plot(t_span(1), W1(i, 1, 2), [w1_colors(i), '-'], 'DisplayName', ['W1', num2str(i),' OMR1 ']);
end
legend;
xlabel('Time (s)');
ylabel('W1 Values');
...
for episode = 1:100
    ...
    for steps = 2:length(t_span)
        for i=1:N
            ...
            if i == 2  % Only update W1 plot for OMR1
                for j = 1:num_weights
                    set(w1_plot_handles(j), 'XData', t_span(1:steps), 'YData', W1(j, 1:steps, 2));
                end
            end
            W1(:,1,i)=W1(:,steps,i);
        end
