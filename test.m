...
%% 更新律设计参数
K1=diag([1,1,1]);
R_u=diag([1,1,1,1,1,1]);
Q_e=diag([1,1,1,1,1,1]);
alpha_w=1;
alpha_e=0.1;
noise_coef=0.1;
...
hat_W=randn(num_weights,1);
% hat_W=rand(6,1);
% 用于保存所有数据的参数
all_hat_W=zeros(num_weights,Episode);
all_z=zeros(num_states,length(t_span));
all_u=zeros(num_inputs,length(t_span));
...
%     z2 = [0.5;0.5;0.5];
    ...
    for steps = 1:length(t_span)
        ...
        if (steps==1) || ((all_E(steps,:)*all_E(steps,:)' - all_E(steps-1,:)*all_E(steps-1,:)') < 0)
            ...
            dot_hat_W=-0.5* alpha_e*dPhi*G(z)*pinv(R_u)*G(z)'*e + alpha_w*(sigma2 / normsigma2^2) * (sigma2' * hat_W + e'*Q_e*e + u' * R_u * u);
        end
        ...
        all_z(:,steps) = z;
        all_u(:,steps) = u;
        value_func(episode,1) = value_func(episode,1) + z'*Q_e*z + u'*u;
    end
